...
class MistralForCausalLM(MistralPreTrainedModel):
    ...
    def _initialize_thought_tokens_if_needed(self):
        # Ensure flags are set before initialization
    ...
    def forward_refactored(
        ...
        self._initialize_thought_tokens_if_needed()
    ...
    def forward(
        ...
        for ahead_idx in range(fwd_iters):
            ...
            if not attempted or self.comparison_mode:
                ...
                # create the updated attention mask for the newly created tokens (this is from the paper, Figure 3 that creates the parallel inference mask)
                # the original attention mask is concatenated with an identity matrix for the newly created tokens
                if len(attention_mask.shape) == 2:
                    ...
                    if self.use_upper_triangular:
                        ...
                        if not attention_mask.dtype == torch.bfloat16:
                            ...
                            logger.warning_once("Using fp32 for attention mask, this might spike memory usage")
                            new_attention = torch.eye(
                                seq_len, dtype=torch.float32, device=attention_mask.device
                            ).to(attention_mask.dtype)
